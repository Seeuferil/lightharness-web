...
인자 없음            → GitHub Issues 조회 (label: harness:pending)
  pending ≥ 2       → 큐 머지 모드
  pending = 1       → 단일 실행 모드
...
GitHub Issues에서 `harness:pending` 라벨 항목을 모두 수집합니다.
각 Issue의 body에서 Blueprint 경로와 Template_Name을 추출합니다.
...
### tier2 (Gemini Flash — MCP)

Gemini MCP 서버가 연결되어 있으면 MCP 도구로 호출합니다.
연결되어 있지 않으면 Tier 1이 직접 처리합니다 (에스컬레이션).
...
각 Blueprint 실행 완료 시:
```
해당 Issue의 label: harness:pending → harness:completed
Issue에 완료 코멘트 추가
```
