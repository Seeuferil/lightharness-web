...
- `w*` 명령어로만 발동: `wharness`, `wharness-run`, `wharness-check`, `wrsm`
- 슬래시 커맨드(/wharness 등) 또는 자연어 트리거 모두 해당
- 스킬 자동 실행 아님 — 명령 수신 시에만 해당 스킬 파일 로드
...
| 세션 종료 | `wrsm log` | 완료 처리 + 다음 할 일 저장 |
